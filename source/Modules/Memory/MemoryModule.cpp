...
bool MemoryModule::Initialize()
{
    ...
    m_resolver = std::make_unique<MemoryResolver>();
...
double MemoryModule::GetValue(uint32_t metricId, uint32_t deviceIndex)
{
...
bool MemoryModule::GetString(uint32_t metricId, uint32_t deviceIndex, std::wstring& out)
{
    return m_resolver ? m_resolver->GetString(metricId, deviceIndex, out) : false;
}

uint32_t MemoryModule::GetDeviceCount() const
{
